...
class OSSHelper {
  ...
  public signatureURL(objKey: string) {
    const client = this._downloadClient as AliyunOSS
    return client.signatureURL(objKey)
  }

  public initSignatureOptions(options: {}) {
  ...
  public generateUploadMetadata(remotePath: string, fileSize: number) {
    ...
    const signOptions = this._signOptions as any
...
/**
 * @deprecated Use OSSUtils Class instead
 */
const ossHelper = new OSSHelper()
export { ossHelper as OSSHelper }
